...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (passwordToggle && passwordInput) {
        passwordToggle.addEventListener('click', () => {
            ...
            const hiddenIcon = passwordToggle.querySelector('.password-hidden-icon');
            const visibleIcon = passwordToggle.querySelector('.password-visible-icon');

            passwordInput.type = passwordIsVisible ? 'password' : 'text';
            passwordToggle.setAttribute(
                'aria-label',
                passwordIsVisible ? 'Tampilkan password' : 'Sembunyikan password',
            );
    ...
    if (loginFeedback) {
        window.setTimeout(() => {
            ...
            document.querySelector('.login-card')?.classList.remove('has-feedback');
