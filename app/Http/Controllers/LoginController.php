...
class LoginController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $allUserMoodleCourses = $this->withCurrentUserCourseProgress(
            $this->withCourseTeachingAccess($this->moodleCoursesForUsername($username)),
        );
        ...
        try {
            ...
            if (($module['modname'] ?? null) === 'assign') {
                ...
                if (! empty($assignment['id'])) {
                    ...
                    $canGradeAssignment = $this->currentUserHasTeachingRole($enrolledUsers);

                    if ($canGradeAssignment) {
                        try {
                            $assignmentGrades = null;
                            try {
                                $assignmentGrades = $this->activeMoodleService()->getAssignmentGrades((int) $assignment['id']);
                            } catch (\Throwable) {
                                $assignmentGrades = null;
                            }

                            $assignmentSubmissions = $this->assignmentSubmissionsForGrading(
                                $this->activeMoodleService()->getAssignmentSubmissions((int) $assignment['id']),
                                $enrolledUsers,
                                $assignmentGrades,
                            );
                        } catch (\Throwable $throwable) {
    ...
    public function gradeAssignmentSubmission(Request $request, int $courseId, int $moduleId)
    {
        if (! session('logged_in')) {
            return redirect()->route('login');
        }
        ...
        try {
            ...
            if (! $module || ($module['modname'] ?? null) !== 'assign') {
                return back()->withErrors(['grade' => 'Aktivitas ini bukan tugas yang dapat dinilai.']);
            }
            ...
            if (empty($assignment['id'])) {
                return back()->withErrors(['grade' => 'Detail tugas tidak ditemukan.']);
            }

            if (! $this->currentUserCanTeachCourse($courseId)) {
                return back()->withErrors(['grade' => 'Akun ini tidak memiliki hak dosen untuk menilai tugas pada course ini.']);
            }

            $this->activeMoodleService()->getAssignmentSubmissions((int) $assignment['id']);

            $maxGrade = max(0, (float) ($assignment['grade'] ?? 100));
            $payload = $request->validate([
                'userid' => ['required', 'integer', 'min:1'],
                'grade' => ['required', 'numeric', 'min:0', 'max:'.$maxGrade],
                'feedback' => ['nullable', 'string', 'max:2000'],
                'workflowstate' => ['required', 'string', Rule::in(array_keys($this->assignmentWorkflowStates()))],
            ], [
                'userid.required' => 'Mahasiswa wajib dipilih.',
                'grade.required' => 'Nilai wajib diisi.',
                'grade.max' => 'Nilai maksimal untuk tugas ini adalah '.$maxGrade.'.',
                'feedback.max' => 'Feedback maksimal 2.000 karakter.',
                'workflowstate.required' => 'Status publikasi nilai wajib dipilih.',
                'workflowstate.in' => 'Status publikasi nilai tidak valid.',
            ]);

            $this->activeMoodleService()->saveAssignmentGrade(
                (int) $assignment['id'],
                (int) $payload['userid'],
                (float) $payload['grade'],
            ...
            $workflowLabel = $this->assignmentWorkflowStates()[$payload['workflowstate']] ?? 'tersimpan';

            return redirect()
                ->route('courses.modules.show', ['courseId' => $courseId, 'moduleId' => $moduleId])
                ->with('success', 'Nilai tugas berhasil disimpan dengan status '.$workflowLabel.'.');
        } catch (\Throwable $throwable) {
            return back()
                ->withInput()
                ->withErrors(['grade' => 'Nilai belum berhasil disimpan. '.$this->moodleUnavailableMessage($throwable)]);
        }
    }
    ...
    protected function withCourseTeachingAccess(mixed $courses): array
    {
        if (! is_array($courses)) {
            return [];
        }
        ...
        return array_map(function ($course): array {
            ...
            $course['can_monitor'] = $courseId > 0 && $this->currentUserCanTeachCourse($courseId);
    ...
    protected function completionProgressByUser(int $courseId, array $students): array
    {
        $progress = [];
        ...
        foreach ($students as $student) {
            ...
            $progress[$userId] = [
                'completed' => $completed,
                'total' => $total,
                'percent' => $total > 0 ? (int) round(($completed / $total) * 100) : null,
            ];
        }
    ...
    protected function courseMonitoringSummary(array $students, mixed $gradesData, mixed $contents, array $completionByUser): array
    {
        $gradeSummaries = $this->gradeSummariesByUser($gradesData);
        $studentRows = [];
        $progressValues = [];
        $gradeValues = [];
        ...
        foreach ($students as $student) {
            $userId = (int) ($student['id'] ?? 0);
            $progress = $completionByUser[$userId] ?? ['completed' => 0, 'total' => 0, 'percent' => null];
            $grade = $gradeSummaries[$userId] ?? [
                'final_grade' => null,
                'graded_items' => 0,
                'total_items' => 0,
                'average' => null,
            ...
            if (($progress['percent'] ?? null) === null && ($grade['total_items'] ?? 0) > 0) {
                $progress = [
                    'completed' => $grade['graded_items'],
                    'total' => $grade['total_items'],
                    'percent' => (int) round(($grade['graded_items'] / max(1, $grade['total_items'])) * 100),
                    'source' => 'grade',
                ];
            } else {
                $progress['source'] = ($progress['percent'] ?? null) === null ? null : 'completion';
            }

            if ($progress['percent'] !== null) {
                $progressValues[] = $progress['percent'];
            }

            if ($grade['final_grade'] !== null) {
                $gradeValues[] = $grade['final_grade'];
            } elseif ($grade['average'] !== null) {
                $gradeValues[] = $grade['average'];
            }
        ...
        return [
            ...
            'activities_count' => $this->courseActivityCount($contents),
            'grade_items_count' => is_array($gradesData) ? count($gradesData['gradeitems'] ?? []) : 0,
            'grade_items' => $this->visibleGradeItems($gradesData),
            'average_progress' => $progressValues === [] ? null : (int) round(array_sum($progressValues) / count($progressValues)),
            'average_grade' => $gradeValues === [] ? null : round(array_sum($gradeValues) / count($gradeValues), 2),
